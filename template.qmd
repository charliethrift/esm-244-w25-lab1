...
* _Answer:_ install and load the "janitor" package. Also, rename the "trees" data to be "sfo_trees", which is a subfolder within data. Then, add the "sfo_trees.csv" file at the end of this path, which is what it is called in the data folder.
```{r}
...
* _Answer:_ There are 197,618 observations in this data frame. Each observation is a separate tree, with columns that describe the location, species, tree id, plot size, and more.

What are some interesting questions you could ask about the data?

Write them below:

* _Answer:_ You could analyze the distribution of different tree species, using the species column and the latitude and longitude.
* You could analyze the distribution of tree species based on the site, which varies by things like sidewalks or yards.
* Below, we determine the count of trees per each legal count.

```{r}
summary(as.factor(sfo_trees_df$legal_status))
...
ggplot(top_5, aes(x=fct_reorder(legal_status,tree_count),y=tree_count))+
...
* _Answer:_ This chunk of code pulls a subset of the "sfo_trees_df" data frame into a new data frame, which includes permitted trees only (from the legal status variable) and trees managed by either MTA or DPW (from the caretaker variable).
...
* _Answer:_ The %in% operator looks for the categories you give it, within whatever object comes before the operator.
...
* subset to just the pine and oak trees from this dataset
...
# also includes Pineapple as a pine
```
...
```{r}
ggplot(oak_pine_df, aes(longitude,latitude))+
  geom_point(aes(color=type,alpha = 0.25))+
  scale_color_manual(values = c("cornflowerblue","seagreen"))+
  theme_bw()+
...
```{r}
native_trees <- oak_pine_df %>% 
    filter(str_detect(species, "Quercus agrifolia") | 
             str_detect(species, "Pinus radiata"))
...
st_transform(sfo_map, 4326) # transform the coordinate reference system to match the one we used above
```
